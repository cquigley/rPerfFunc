...
contr_cap <- function(contr_data){
  #contr_data<-'BUCKYTPS_S'
  memb_contr = get(contr_data)
  memb_contr$date = as.Date(memb_contr$date)
  df = memb_contr[(memb_contr$date>start_date & memb_contr$date<= end_date), ]
  df$ticker = as.vector(df$ticker)
  ...
  result_temp$ticker<-rownames(result_temp)
  result_temp = result_temp[rownames(result_temp)!= 'Total', ]
  cashdf = result_temp[rownames(result_temp)=='Cash', ]
  result_temp = result_temp[rownames(result_temp)!= 'Cash', ]
  result_temp$ticker<-gsub("\\.", "/", result_temp$ticker) 
  result_temp$ticker = paste(result_temp$ticker, " US Equity", sep = "")
  capdf = bdh(conn, as.vector(result_temp$ticker), "CUR_MKT_CAP", trade.day, trade.day)
  colnames(capdf)[3]<-"Cap"
  result_temp<-merge(result_temp, capdf, by.y = 'ticker')
  #result_temp$ticker<-NULL
  result_temp$date<-NULL
  result_temp$Cap<-as.numeric(result_temp$Cap)
  #result_temp = result_temp[complete.cases(result_temp), ]
  result_temp = result_temp[result_temp[, 'avg_wgt']>.01, ]
  for (i in 1:nrow(result_temp)){
    result_temp$capclass[i]<-
      ifelse(result_temp$Cap[i]>0 && result_temp$Cap[i]<= 300, "Micro", 
             ifelse(result_temp$Cap[i]>300 && result_temp$Cap[i]<= 2000, "Small", 
                    ifelse(result_temp$Cap[i]>2000 && result_temp$Cap[i]<= 10000, "Medium", 
                           ifelse(result_temp$Cap[i]>10000 && result_temp$Cap[i]<= 50000, "Large", 
                                  ifelse(result_temp$Cap[i]>50000, "Giant", 
                                         'Unclassified')))))
  } 
  result_temp$Cap<-NULL
  result_temp<-rbind.fill(result_temp,cashdf)
  result_temp$capclass[is.na(result_temp$capclass)]<-"Unclassified"
  result_temp = result_temp[order(result_temp[, 'contr'], decreasing = TRUE), ]
  result = data.frame(Ticker = gsub(" US Equity", "", result_temp$ticker), TR=result_temp$TR,
                      Contribution = result_temp$contr, 'Average Weight' = result_temp$avg_wgt, 
                      'Class' = result_temp$capclass, check.names = FALSE)
  
  resulta<-cast(result, Class~., value = 'Contribution', sum, na.rm = TRUE)
  resultb<-cast(result, Class~., value = 'Average Weight', sum, na.rm = TRUE)
  resultc<-cast(result, Class~., value = 'TR', mean, na.rm = TRUE)
  
  result<-data.frame(Class=resulta[,1],Contribution=resulta[,2],
                     'Average Weight'=resultb[,2],'Total Return'=resultc[,2])
  
  colnames(result)<-c("Market Cap", "Contribution", "Average Weight","Total Return")
  result$'Average Weight'[nrow(result)]<-(100-(sum(result$'Average Weight')-result$'Average Weight'[nrow(result)]))
  resultsum<-data.frame('Market Cap' = 'Total', 
                        Contribution = sum(result$Contribution), 
                        'Average Weight' = sum(result$'Average Weight'),
                        'Total Return' = mean(result$'Total Return'))
  colnames(resultsum)<-colnames(result)
...
#' CRP_blend_contr <- contr_bench_IDX(start_date = "2009-12-31", 
#'                                    end_date = "2013-12-31", 
#'                                   eq_ret = "NDUEACWZ_rets", 
#'                                   fi_ret = "BARCAPAGG_rets", mkt_pct = 0.6, 
#'                                   tickers = c("NDUEACWZ", "LBUSTRUU"))
